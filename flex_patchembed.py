...
class STAPE4D_TimeToSpace(nn.Module):
    ...
    def _add_positional_encoding(self,
        ...
        return tokens_with_pos, pos_group

    # ---- 核心：组内处理（时间→通道折叠→空间）----
    def _run_group_time_first(
        self,
        ...
        # 统一 pad 到组内一致的 T_pad(kt 的倍数) 与 T'
        T_true_max = max(orig_Ts) if len(orig_Ts)>0 else 0
        T_pad = math.ceil(T_true_max / kt) * kt if T_true_max>0 else 0
        T_prime = (T_pad // kt) if T_pad>0 else 0

        # 边界：该组样本全为 0 帧
        if T_prime == 0:
            tokens_list, lengths, grid_data = [], [], {}
            for g in range(G):
                keep_mask = self._spatial_keep_mask_alltime(x_group[g], kx,ky,kz, 0)
                tokens_list.append(x_group.new_zeros((0, self.Do)))
                lengths.append(0)
                if return_grid_info:
            ...
            return tokens_list, lengths, grid_data

        # ---- pad/crop到 T_pad ----
        xg = x_group
        if T_max < T_pad:
            xg = F.pad(xg, (0, T_pad - T_max), mode='constant', value=0.0)
        xg = xg[..., :T_pad]  # [G,96,96,96,T_pad]

        # ---- 时间 STAPE：Conv1d stride=kt（不折到通道）----
        xlin = xg.view(G*X*Y*Z, 1, T_pad)
        w_t = self._get_wt_first(kt, device, dtype)                 # [Dm,1,kt]
        b_t_first = self.b_t_first.to(device=device, dtype=dtype)
        tfeat = F.conv1d(xlin, w_t, bias=b_t_first, stride=kt)      # [G*X*Y*Z, Dm, T′]
        tfeat = tfeat.view(G, X, Y, Z, self.Dm, T_prime) \
                    .permute(0,5,4,1,2,3).contiguous()             # [G, T′, Dm, X, Y, Z]

        # ---- 空间 STAPE：把 T′ 并到 batch ----
        x_sp_in = tfeat.view(G*T_prime, self.Dm, X, Y, Z)           # [G*T′, Dm, X,Y,Z]
        w_xyz = self._get_wxyz_after(kx,ky,kz, device, dtype)       # [Do, Dm, kx,ky,kz]
        b_xyz_after = self.b_xyz_after.to(device=device, dtype=dtype)
        sfeat = F.conv3d(x_sp_in, w_xyz, bias=b_xyz_after, stride=(kx,ky,kz))  # [G*T′, Do, Lx,Ly,Lz]
        sfeat = sfeat.view(G, T_prime, self.Do, Lx, Ly, Lz)         # [G, T′, Do, Lx, Ly, Lz]

        # ---- 展平成 (B, T' * Lx*Ly*Lz, D) 的 token 轴 ----
        # 先变为 [G, T′, L, Do] 便于后续用空间 keep_mask 筛选
        tokens_all_time = sfeat.permute(0,1,3,4,5,2).contiguous().view(G, T_prime, L, self.Do)  # [G, T′, L, Do]

        # ---- 位置编码：仅加空间PE（对时间维广播）----
        coords = self._compute_spatial_coords_for_group(group_idxs, affines, kx, ky, kz, device)  # [G, L, 3]
        pos = self.pos_embed(coords).to(dtype)                                                   # [G, L, Do]
        tokens_all_time = tokens_all_time + pos.unsqueeze(1)                                     # [G, T′, L, Do]

        # ---- 依据每样本 T'_i 与空间 keep_mask 取有效 token ----
        tokens_list, pos_list, lengths, grid_data = [], [], [], {}
        for g in range(G):
            T_true = orig_Ts[g]
            T_prime_i = math.ceil(T_true / kt) if T_true>0 else 0

            keep_mask = self._spatial_keep_mask_alltime(x_group[g], kx,ky,kz, T_true)  # [L]
            if T_prime_i>0 and keep_mask.any():
                # [T'_i, L, Do] -> 选空间 -> [T'_i, N_sp, Do] -> 展平为 [T'_i*N_sp, Do]
                toks = tokens_all_time[g, :T_prime_i]                          # [T'_i, L, Do]
                toks = toks[:, keep_mask, :]                                   # [T'_i, N_sp, Do]
                toks = toks.reshape(-1, self.Do)                                # [T'_i*N_sp, Do]

                pe = pos[g, keep_mask,, :].unsqueeze(0).expand(T_prime_i, -1, -1)  # [T'_i, N_sp, Do]
                pe = pe.reshape(-1, self.Do)                                    # [T'_i*N_sp, Do]
            else:
                toks = tokens_all_time.new_zeros((0, self.Do))
                pe = tokens_all_time.new_zeros((0, self.Do))
    ...
    def forward(self,
        ...
        patch_sizes: List[Tuple[int,int,int,int]] = [(0,0,0,0)] * B  # (kx,ky,kz,kt)
        ...
        for (voxel, tr), idxs in groups.items():
            ...
            for loc, (tok, ln) in zip(idxs, zip(toks, lens)):
                ...
                per_sample_pos[loc] = pos_list[loc]
                lengths[loc] = ln
                patch_sizes[loc] = (kx, ky, kz, kt)
        ...
        if return_grid_info:
            return out, attn_mask, lengths, grid_info, patch_sizes
        else:
            return out, attn_mask, lengths, patch_sizes
