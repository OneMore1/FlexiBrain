    ...
    def forward(
        ...
        if input_dtype == torch.float32:
            if torch.is_autocast_enabled():
                ...
                # Use fp16 as default for fp32 inputs
                target_dtype = torch.float16

            # Suppress the warning for fp32 inputs - this is expected behavior
            # logger.warning(...)
        ...
        # Convert back to original dtype if it was converted for FlashAttention
        if input_dtype == torch.float32 and attn_output.dtype != torch.float32:
...
class Block(nn.Module):
    def __init__(
        ...
        super().__init__()
        # Check if norm_layer is a callable (function) or a class
        # If it's a function, call it; if it's a class, instantiate it
        if callable(norm_layer) and not isinstance(norm_layer, type):
            # It's a function (like norm_layer_with_dtype)
            self.norm1 = norm_layer(dim)
            self.norm2 = norm_layer(dim)
        else:
            # It's a class (like nn.LayerNorm)
            self.norm1 = norm_layer(dim)
            self.norm2 = norm_layer(dim)
