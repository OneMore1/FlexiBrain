...
class Block(nn.Module):
    def __init__(
        ...
        # === 记录该 mixer.forward 支持的参数名 ===
        try:
            self._mixer_kwset = set(inspect.signature(self.mixer.forward).parameters.keys())
        except Exception as e:
            # 避免某些 C++/CUDA 模块无 signature 的情况
            print(f"Warning: Failed to get mixer signature: {e}")
            # 对于Mamba，我们知道它支持这些参数
            if hasattr(self.mixer, 'forward') and 'Mamba' in str(type(self.mixer)):
                self._mixer_kwset = {'hidden_states', 'inference_params', 'attn_mask'}    # attn_mask
                print(f"Using default Mamba parameters: {self._mixer_kwset}")
            else:
    ...
    def forward(
        ...
        if not self.fused_add_norm:
         ...
         # 归一化后，准备把 kwargs 传给 mixer 前，先过滤一遍
        if self._mixer_kwset:
            filtered_kwargs = {k: v for k, v in mixer_kwargs.items() if k in self._mixer_kwset}
        else:
            # 无法探测签名时，保守起见不传任何额外 kwargs，避免 TypeError
            filtered_kwargs = {}
