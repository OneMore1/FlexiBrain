...
class Mamba(nn.Module):
    ...
    def _right_pad_lengths(attn_mask: Tensor) -> Tuple[Tensor, int]:
        ...
        """
        seqlens = (~attn_mask).sum(dim=1)  # (B,)
        L_eff = int(seqlens.max().item())
        return seqlens, L_eff

    def pack_ragged(self, tokens, attn_mask):
        # tokens: (B, Lmax, D), attn_mask True=PAD
        B, Lmax, D = tokens.shape
        keep = (~attn_mask).view(B * Lmax)             # (B*Lmax,)
        idx  = torch.nonzero(keep, as_tuple=False).squeeze(-1)  # 有效位的扁平索引
        flat = tokens.view(B * Lmax, D).index_select(0, idx)    # (sum(Lb), D)

        seqlens = (~attn_mask).sum(dim=1).to(torch.int32)       # (B,)
    ...
    # ----------------- 修改后的 forward：支持右侧 attn_mask -----------------
    def forward(
        self,
        hidden_states: Tensor,                      # (B, Lmax, D)
        inference_params=None,
        attn_mask: Optional[Tensor] = None,         # (B, Lmax) True=PAD（右侧）
    ) -> Tensor:
        """
        并行计算；为 PAD 付费，但：
          1) PAD 位置输出强制置 0；
        ...
        # 批内裁剪公共右端全 PAD（按 max(seqlens) 裁到 L_eff），一次 dense 并行
        x_eff = hidden_states[:, :L_eff, :]                      # (B, L_eff, D)
        y_eff = self.forward_dense(x_eff, None)       # (B, L_eff, D)

        # 还原到 (B, Lmax, D)，右侧补 0
        if L_eff < Lmax:
            pad_right = Lmax - L_eff
            y = F.pad(y_eff, (0, 0, 0, pad_right))               # (B, Lmax, D)
        else:
            y = y_eff

        # 安全起见：PAD 位强制置零（确保不计入有效结果）
